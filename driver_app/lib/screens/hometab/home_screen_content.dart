...
class _HomeScreenContentState extends State<HomeScreenContent> {
  ...
  Future<void> _fetchAllocationVehicle() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$serverUrl/api/v1/driver/allocation?driverId=$driverId'),
      );
  ...
  Future<void> _fetchHourlySchedule() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
          '$serverUrl/api/v1/schedules?driverId=$driverId&dispatchTime=${now.toIso8601String()}',
        ),
      );
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['success'] == true &&
          ...
          setState(() {
            ...
            campusReturnTime = DateTime.tryParse(
  ...
  Future<void> _handleShiftToggle() async {
    ...
    try {
      final serverUrl = dotenv.env['SERVER_URL'];
      await http.post(
        Uri.parse('$serverUrl/api/v1/driver/journey'),
